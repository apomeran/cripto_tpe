...
int checkLinealDependency(int, int, byte, byte, image_t**, int);
...
void k_2_encode(image_t** shadows, image_t* secret, int shadow_count) {
    ...
    for (index = 0; index < img_size; index += 2) {
		...
		for (current_shadow = 0; current_shadow < shadow_count; current_shadow++) {
            ...
            while (checkLinealDependency(index, current_shadow, decal_first_shadow_byte, decal_second_shadow_byte, shadows, shadow_count) == 1) {
				count++;
...
int checkLinealDependency(int index, int current_shadow, byte first_byte, byte second_byte, image_t** shadows, int n) {
    int i;
